...
import { commandDef as configCmd } from './config.js'
import { commandDef as cnameCmd } from './cname.js'

export const mgrGroup: Group = {
  name: 'mgr',
  alias: 'm',
  description: '统一管理器：注册 npm / py / exe 项并通过别名调用',
  commands: [addCmd, installCmd, listCmd, runCmd, rmCmd, renameCmd, checkCmd, exportCmd, importCmd, backupCmd, restoreCmd, configCmd, cnameCmd],
}